...
Large-scale Item-response Statistical Table Constructor

状态:草案(2026-07-18) 负责人:Ma Kunxiang

## 1. 定位与创新点

LISTC 是一个把测评/调查样本数据转化为可自由定制的透视统计表的 R 包。
...
| 懂 R 的统计人员 | `lst_data() → st_* → lst_table()` 完整函数 API | `listc_table` 对象、tidy 长表、SE 全分量 |
...
`listc_data` = data.frame + 变量角色说明(spec)。角色:
...
R/
  ...
  table.R           lst_table(), listc_table 对象
  pivot.R           长表↔宽表
...
- 按需加载列:`read_listc` 支持 `col_select`,`lst_run` 从
